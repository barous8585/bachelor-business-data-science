...
with tab1:
    ...
    if not flashcards:
        ...
        if matiere_filter == "Toutes":
            ...
            cards_to_review = [fc for fc in flashcards if fc['matiere'] == matiere_filter]
                ...
                if not st.session_state.show_answer:
                    ...
                    with col1:
                        if st.button("❌ Difficile", use_container_width=True):
                            current_card['difficulte'] = 'difficile'
                            current_card['dernier_revu'] = str(datetime.now().date())
                    ...
                    with col3:
                        if st.button("✅ Facile", use_container_width=True):
                            current_card['difficulte'] = 'facile'
                            current_card['dernier_revu'] = str(datetime.now().date())
                            st.session_state.current_card_index += 1
                            st.session_state.show_answer = False
                            save_flashcards(flashcards)

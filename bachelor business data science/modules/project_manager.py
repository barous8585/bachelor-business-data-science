        ...
        for project in projects:
            ...
            with st.expander(f"{status_color.get(project['status'], '⚪')} {project['nom']} - {project['type']}"):
                ...
                with col1:
                    ...
                    st.markdown(f"**Début :** {project['date_debut']}")
                    if project.get('date_fin'):
                        st.markdown(f"**Fin :** {project['date_fin']}")
                ...
                if 'taches' in project:
                    for tache in project['taches']:
                        done = "✅" if tache.get('done', False) else "⬜"
                        st.markdown(f"{done} {tache['nom']}")
                
                st.markdown("---")
                col1, col2, col3 = st.columns(3)
                
                with col1:
                    if st.button("✏️ Modifier", key=f"edit_{project['nom']}"):
                        st.session_state['editing_project'] = project
                
                with col2:
                    if st.button("🗑️ Supprimer", key=f"delete_{project['nom']}"):
    ...
    if st.button("💾 Enregistrer le projet"):
        if nom:
            new_project = {
                ...
                'taches': st.session_state.task_list
            }
